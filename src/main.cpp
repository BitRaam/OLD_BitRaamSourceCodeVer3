...
// Copyright (c) 2013 BitRaam developers
...
const string strMessageMagic = "BitRaam Signed Message:\n";
...
/** Amount of bitraams spent by the transaction.
...
static const int64 nTargetTimespan = 240; // 2 minutes
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
	...
	if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) { 
        ...
        int64 LatestBlockTime = BlockLastSolved->GetBlockTime();  //KGW Exploit Patch    
	for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
		...
		if (LatestBlockTime < BlockReading->GetBlockTime()) {
			if (BlockReading->nHeight > 45000) // HARD Fork block number
...
{
	static const int64 BlocksTargetSpacing = 1 * 120; // 60 seconds
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitRaamMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitRaamMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitraamMiner(CWallet *pwallet)
{
    printf("BitRaamMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitraam-miner");
    ...
    try { loop {
        ...
        printf("Running BitRaamMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitRaamMiner terminated\n");
        throw;
    }
}

void GenerateBitraams(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitraamMiner, pwallet));
}
