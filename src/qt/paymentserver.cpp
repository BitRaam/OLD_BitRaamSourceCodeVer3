...
const int BITRAAM_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BITRAAM_IPC_PREFIX("bitraam:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(BITRAAM_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BITRAAM_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(BITRAAM_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click bitraam: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start bitraam: click-to-pay handler");
    else
...
{
    // clicking on bitraam: URLs creates FileOpen events on the Mac:
